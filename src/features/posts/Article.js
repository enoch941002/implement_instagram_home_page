...
import { newComment } from 'features/posts/postsSlice'
const Article = () => {
    ...
    return (
        <article>{
            posts.map((item, i) => {
                ...
                const combineString = `${date.day === 0 ? (`${date.hour === 0 ? (`${date.mins === 0 ? (`${date.sec === 0 ? '' : `${date.sec}秒`}`) : `${date.mins}分`}`) : `${date.hour}小時`}`) : `${date.day}天`}`
                return (
